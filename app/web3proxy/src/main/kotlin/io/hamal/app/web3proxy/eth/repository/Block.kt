...
interface EthBlockRepository {
    fun list(blockNumbers: List<EthUint64>): List<EthBlock?>
}

class EthBlockRepositoryImpl(
    path: Path,
    ...
    private fun findBlock(blockNumber: EthUint64): EthBlock? {
        return connection.tx {
            ...
            val addresses = addressRepository.list(collectEthAddressIds(listOf(entity)))
            entity.toObject(addresses)
        }
    }

    private fun insertBlocks(blocks: List<EthBlock>) {
        val addresses = addressRepository.resolve(collectEthAddresses(blocks))
        val entities = blocks.map { it.toEntity(addresses) }

        connection.tx {
            entities.forEach { entity ->
                execute("INSERT OR REPLACE INTO block(number, hash, data) VALUES( :number, :hash, :data )") {
                    set("number", entity.number.value)
                    set("hash", entity.hash.toPrefixedHexString().value)
                    set("data", json.serializeAndCompress(entity))
                }
            }
        }
    }
