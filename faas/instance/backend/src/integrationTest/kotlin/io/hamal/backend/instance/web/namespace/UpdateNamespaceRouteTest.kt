...
internal class UpdateNamespaceRouteTest : BaseNamespaceRouteTest() {
    ...
    fun `Updates namespace`() {
        val namespace = awaitCompleted(
            createNamespace(
                CreateNamespaceReq(
                    ...
                    inputs = NamespaceInputs(DepTableType("hamal" to StringType("createdInputs")))
                )
            )
        )
        ...
        val updateNamespaceResponse = httpTemplate.put("/v1/namespaces/{namespaceId}")
            ...
            .body(
                UpdateNamespaceReq(
                    ...
                    inputs = NamespaceInputs(DepTableType("hamal" to StringType("updatedInputs")))
                )
            )
            .execute()
        ...
        with(getNamespace(namespaceId)) {
            ...
            assertThat(inputs, equalTo(NamespaceInputs(DepTableType("hamal" to StringType("updatedInputs")))))
        }
    }
}