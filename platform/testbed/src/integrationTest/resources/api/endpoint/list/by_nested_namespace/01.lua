...
sys.await_completed(fail_on_error(sys.endpoints.create({ namespace_id = namespace_one_req.id, func_id = func_one.func_id, name = 'endpoint-1' })))
...
sys.await_completed(fail_on_error(sys.endpoints.create({ namespace_id = namespace_two_req.id, func_id = func_two.func_id, name = 'endpoint-2' })))
