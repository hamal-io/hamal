...
internal class SerdeArrayTest {

    @TestFactory
    fun `is`() = listOf(
        Tuple3("isArray", SerdeArray.empty.isArray, true),
        Tuple3("isBoolean", SerdeArray.empty.isBoolean, false),
        Tuple3("isNumber", SerdeArray.empty.isNumber, false),
        Tuple3("isNull", SerdeArray.empty.isNull, false),
        Tuple3("isObject", SerdeArray.empty.isObject, false),
        Tuple3("isString", SerdeArray.empty.isString, false),
        Tuple3("isTerminal", SerdeArray.empty.isPrimitive, false)
    ...
    inner class AsTest {
        ...
        @TestFactory
        fun `as`() = listOf(
            Tuple3("asArray", testInstance.asArray(arrayIdx), SerdeArray.builder().append(42).build()),
            Tuple3("asBoolean", testInstance.asBoolean(booleanIdx), SerdeBoolean(true)),
            Tuple3("asNumber", testInstance.asNumber(numberIdx), SerdeNumber(123)),
            Tuple3("asNull", testInstance.asNull(nullIdx), SerdeNull),
            Tuple3("asNull - does not found", testInstance.asNull(doesNotExistIdx), SerdeNull),
            Tuple3("asObject", testInstance.asObject(objectIdx), SerdeObject.builder().set("hamal", "rocks").build()),
            Tuple3("asString", testInstance.asString(stringIdx), SerdeString("SomeString")),
            Tuple3("asTerminal", testInstance.asTerminal(stringIdx), SerdeString("SomeString")),
        ).map { (testName, result, expected) ->
            dynamicTest(testName) {
                assertThat(result, equalTo(expected))
            }
    ...
    inner class ValueTest {
        ...
        fun `value - wrong type`() = listOf<Tuple4<kotlin.String, Int, (Int) -> Any, kotlin.String>>(
            ...
            Tuple4("decimalValue", booleanIdx, testInstance::decimalValue, "Not HotNumber"),
    ...
    @Test
    fun empty() {
        val testInstance = SerdeArray.empty
    ...
    inner class FindTest {
        ...
        fun `Finds key`() {
            ...
            assertThat(result, equalTo(SerdeString("A")))
        }
        ...
        private val testInstance: SerdeArray = SerdeArray.builder().append("A").build()
    }
    ...
    inner class DeepCopyTest {
        ...
        fun `Copy object`() {
            ...
            assertThat(result.asArray(arrayIdx), equalTo(SerdeArray.builder().append(42).build()))
            assertThat(result.asBoolean(booleanIdx), equalTo(SerdeBoolean(true)))
            assertThat(result.asNumber(numberIdx), equalTo(SerdeNumber(123)))
            assertThat(result.asString(stringIdx), equalTo(SerdeString("SomeString")))
            assertThat(result.asObject(objectIdx), equalTo(SerdeObject.builder().set("hamal", "rocks").build()))
            assertThat(result.asNull(nullIdx), equalTo(SerdeNull))
        }
    }

}

private val testInstance = SerdeArray.builder()
    .append(SerdeArray.builder().append(42).build())
    .append(true)
    .append(123)
    .append("SomeString")
    .append(SerdeObject.builder().set("hamal", "rocks").build())
    .appendNull()
    .build()
