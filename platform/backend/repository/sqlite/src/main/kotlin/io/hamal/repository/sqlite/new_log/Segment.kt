...
class LogSegmentSqliteRepository(
    ...
    override fun append(cmdId: CmdId, bytes: ByteArray) {
        connection.tx {
            execute("INSERT OR IGNORE INTO events (cmd_id,bytes,instant) VALUES (:cmdId, :bytes,:now) RETURNS id") {
                set("cmdId", cmdId)
                set("bytes", bytes)
    ...
    override fun count(): Count = connection.executeQueryOne("SELECT COUNT(*) as count from chunks") {
        map {
