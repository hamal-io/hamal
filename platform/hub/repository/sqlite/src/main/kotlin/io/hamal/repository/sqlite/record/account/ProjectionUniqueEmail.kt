...
internal object ProjectionUniqueEmail : Projection<AccountId, AccountRecord, Account> {

    override fun upsert(tx: RecordTransaction<AccountId, AccountRecord, Account>, obj: Account) {
        val email = obj.email
        if (email != null) {
            try {
                tx.execute(
